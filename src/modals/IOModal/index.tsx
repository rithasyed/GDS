...
export default function IOModal({
  ...
  const fileinputs = useFlowStore((state) => state.fileinputs)
  console.log("fileinputs",fileinputs)
  // const fileinputs = useFlowStore((state) => state.fileinput).filter(
  //   (input) => fileinputs.type == "Chatinput"
  // );
  const nodes = useFlowStore((state) => state.nodes).filter(
    (node) =>
      inputs.some((input) => input.id === node.id) ||
      outputs.some((output) => output.id === node.id)||
      fileinputs.some((fileinput) => fileinput.id === node.id)
  );
            ...
            {selectedTab !== 0 && (
              ...
              >
                ...
                >
                  ...
                  >
                    <div className="mx-2 mb-2 flex items-center gap-2 text-sm font-bold">
                      <IconComponent className="h-4 w-4" name={"File"} />
                      {FILE_INPUT_MODAL_TITLE}
                    </div>
                    {nodes
                      .filter((node) =>
                      ...
                      .map((node, index) => {
                        const fileinput = fileinputs.find(
                          (fileinput) => fileinput.id === node.id
                        )!;
                        ...
                        return (
                          ...
                          >
                            <AccordionComponent
                              ...
                              key={index}
                              keyValue={fileinput.id}
                            >
                              <div className="file-component-tab-column">
                                <div className="">
                                  {fileinput && (
                                    <IOFieldView
                                      type={InputOutput.FILE}
                                      left={true}
                                      fieldType={fileinput.type}
