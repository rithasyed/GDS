...
import { isFileNode, isInputNode, isOutputNode } from "./reactflowUtils";
...
export function getInputsAndOutputs(nodes: Node[]) {
  ...
  let fileinputs: { type: string; id: string; displayName: string }[] = [];
  ...
  return { inputs, outputs, fileinputs };
}
